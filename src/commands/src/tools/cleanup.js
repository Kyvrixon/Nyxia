...
export default async (client, interaction) => {
	if (
		...
		return await interaction.reply({
			embeds: [
				errEmbed(
					"Seems like you or I do not have permission to delete messages in this channel",
					null,
					interaction,
					"No permission"
				),
	...
	if (
		(!/^[1-9][0-9]?$|^100$/.test(amountStr) && amount !== null) ||
		!parseInt(amountStr)
	) {
	...
	try {
		if (!target) {
			...
			if (botMessages.size > 0) {
				const done = await interaction.channel.bulkDelete(
					botMessages,
					true
				);
