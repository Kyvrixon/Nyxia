...
export default async (client) => {
	...
	try {
		const response = await fetch(
			`https://discord.com/api/v10/applications/${process.env.BOT_ID}/emojis`,
			{
				headers: {
		...
		if (!Array.isArray(existingEmojis)) {
			Logger.error(
				"Emoji Uploader",
				"Expected an array of emojis but got: " +
		...
		if (existingEmojis.length === 0) {
			Logger.info(
				"Emoji Uploader",
				"No existing emojis found. Uploading all emojis from emojisList."
			);
		}

		const existingNames = new Set(
			existingEmojis.map((emoji) => emoji.name)
		);
		const uniqueEmojis = {};
			...
			for (const emojiName in uniqueEmojis) {
				...
				try {
					...
					if (!imageResponse.ok) {
						Logger.error(
							"Emoji Uploader",
							`Failed to fetch image for ${emojiName}: ${imageResponse.statusText}`
						);
						continue;
					}
					...
					if (!uploadResponse.ok) {
						...
						Logger.error(
							"Emoji Uploader",
							`Failed to upload emoji ${emojiName}: ${error.message}`,
							error
						);
					}
					...
					Logger.error(
						"Emoji Uploader",
						`Failed to fetch image for ${emojiName}: ${imageFetchError.message}`,
						imageFetchError
					);
				}
			}
