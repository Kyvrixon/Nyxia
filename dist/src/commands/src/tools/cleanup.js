...
export default async (client, interaction) => {
    ...
    if ((!/^[1-9][0-9]?$|^100$/.test(amountStr) && amount !== null) ||
        !parseInt(amountStr)) {
        return interaction.reply({
            embeds: [
                errEmbed(`Your amount must be a whole number between **1** and **100**. It cannot be \`${amountStr}\`.`, null, interaction, "Invalid input"),
            ],
            ephemeral: true,
        });
    }
    ...
    if (amount > 100 || amount < 1) {
        return interaction.reply({
            embeds: [
                errEmbed(`Your amount must be between **1** and **100**. It cannot be \`${amount}\`.`, null, interaction, "Invalid input"),
            ],
            ephemeral: true,
        });
    }
    ...
    await interaction.reply({
        embeds: [
            basicEmbed(null, "> Cleaning messages...", null, null, null, null, null, null, null),
        ],
    });
    ...
    try {
        ...
        const confirmationMsg = await interaction.channel.send({
            embeds: [
                basicEmbed("Cleanup Completed!", `${totalMsgs ? `Done! I have cleared \`${msgCount}/${totalMsgs}\` messages!` : "Hm, no messages for me to clear here!"}`, null, "Green", null, "I will auto delete in 5 seconds", null, null, null),
            ],
        });
        setTimeout(async () => {
            try {
