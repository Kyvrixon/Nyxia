...
export const createLeaderboard = async (title, txt, interaction, pageCount = 10, extra_components = null, footerText = null) => {
    ...
    const createButton = (id, label, disabled = false) => new Discord.ButtonBuilder()
        .setCustomId(id)
        .setLabel(label)
    ...
    try {
        ...
        return await interaction?.channel?.send({
            embeds: [
                errEmbed("Something went wrong while trying to initialise a module", e, interaction),
            ],
        });
    }
    // safeguard
        ...
        if (btn.user.id ===
            ...
            if (btn.customId === "page_info") {
                const modal = new Discord.ModalBuilder()
                    ...
                    .addComponents(new Discord.ActionRowBuilder().addComponents(new Discord.TextInputBuilder()
                ...
                if (modalSubmit) {
                    ...
                    if (isNaN(pageNumber) ||
                        ...
                        const row2 = new Discord.ActionRowBuilder().addComponents(createButton("back_button", "prev", currentIndex === 0), createButton("page_info", `${pageNumber}/${totalPages}`, totalPages === 1), createButton("forward_button", "next", currentIndex + (single ? 1 : pageCount) >=
                            lb.length));
                        await Promise.all([
                            msg.edit({
                                embeds: [
                                    await generateEmbed(currentIndex, lb, title),
                                ],
                ...
                const row2 = new Discord.ActionRowBuilder().addComponents(createButton("back_button", "prev", currentIndex === 0), createButton("page_info", `${Math.floor(currentIndex / (single ? 1 : pageCount)) + 1}/${totalPages}`, totalPages === 1), createButton("forward_button", "next", currentIndex + (single ? 1 : pageCount) >= lb.length));
                await Promise.all([
                    msg.edit({
    ...
    collector.on("end", async () => {
        const rowDisable = new Discord.ActionRowBuilder().addComponents(createButton("expired_button", "This component has expired!", true));
        try {
            await msg.edit({
                components: extra_components
...
export const devCheck = async (input) => {
    const userId = typeof input === "bigint" || typeof input === "number"
        ? input.toString()
        : typeof input === "string"
            ? input
            : false;
...
export const getInvite = async (guild, channel) => {
    try {
        ...
        if (invites.length > 0) {
            invite = invites.find((invite) => invite.inviter &&
        ...
        if (!invite && !channel) {
            const targetChannel = guild.systemChannel || // System channel (if set)
