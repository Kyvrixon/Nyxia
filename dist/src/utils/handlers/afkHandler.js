        ...
        if (isInAfk.message) {
            try {
                msg1 = await message.channel.send({
                    embeds: [
                        basicEmbed(null, "You was AFK since <t:" +
                        ...
                        }, "This msg will autodelete in 15s", null, null, null),
                    ],
                });
            }
            catch { }
            ...
            if (!message.content.includes("@here") &&
                ...
                if (isAFK.message) {
                    msg2 = await message.channel.send({
                        embeds: [
                            basicEmbed(null, `**Reason:** ${isAFK.message}\n**Since:** <t:${Math.floor(isAFK.time / 1000)}:R>`, null, "DarkButNotBlack", {
                                name: u.username + " is currently AFK!",
                                iconURL: u.displayAvatarURL({
                                    dynamic: true,
                                }),
                            }, "This msg will autodelete in 15 seconds", new Date().now),
                        ],
                    });
                    if (msg2) {
                        setTimeout(async () => {
